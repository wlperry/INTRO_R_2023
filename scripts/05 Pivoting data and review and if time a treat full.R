...
data.df <- read_excel("data/data_r.xlsx") %>% clean_names()
...
data.df %>% 
  ggplot() +
  geom_boxplot(aes(x=1, y=wt), color="blue") +
  geom_boxplot(aes(x=2, y=las_i), color= "red")

# so its hard to plot in wide format - how do we switch from wide to long to wide again?-----
# To long format -----
data_long.df <- data.df %>% 
  pivot_longer(
    cols = c(pos_cont, wt, ppk1, ppx, las_i),
    names_to = "strain",
    values_to = "value")
